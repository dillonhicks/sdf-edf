...
sdf_edf_member_deadline_timer_callback(struct hrtimer *mem_timer) {
	...
	case SDF_EDF_MEM_RUNNING:
		...
		/* If the member is a group that has missed its
		 * deadline there will be no task to which to send the
		 * signal. Therefore, short the routine to the
		 * timerforward section.
		 */
		if (unlikely(md->member->type == GSCHED_COMPTYPE_GROUP)) {
			printk("%s:%d:WARNING: HGS EDF group missed deadline [%s]\n", 
			       __FILE__, __LINE__, md->member->ns->name);
			goto out;						
		}
		
		mem_task = md->member->task.ptr;

		/* send the signal */
		memset(&info, 0, sizeof(struct siginfo));
		
		info.si_signo = SIG_MISSED_DEADLINE;
		/* this is bit of a trickery: SI_QUEUE is normally
		 * used by sigqueue from user space, and kernel space
		 * should use SI_KERNEL. But if SI_KERNEL is used the
		 * real_time data is not delivered to the user space
		...
		//send the signal
		ret = send_sig(SIG_MISSED_DEADLINE, mem_task, false);
		if (ret < 0) {
			printk("%s:%d:ERROR: sending missed deadline signal\n", 
			       __FILE__, __LINE__);			
		}
		break;
