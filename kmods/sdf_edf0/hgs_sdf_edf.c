...
static void __sdf_edf_set_member_status(struct edf_group *gd, struct edf_member *md, int next_status);
...
{
	DSTRM_EVENT(SDF_EDF, FUNC_ITERATOR_PREPARE, 0);
				 ...
				 struct edf_cpu *cd, int cpu)
{
	...
	DSTRM_EVENT(SDF_EDF, FUNC_FIND_EARLIEST_DL, 0);
	switch(gd->status){
	case SDF_EDF_GROUP_INIT:
		if (prev)
		{
			md = prev->sched_data;
			mem_cpu = &md->cpu[cpu];
	...
	case SDF_EDF_GROUP_RUNNING:
		if (prev)
		{
			...
			list_for_each_entry_continue(mem_cpu, &cd->sorted_members, sorted_ent) {
				 ...
				 * the current epoch.
				 */
				if ((!next || md->deadline < next->deadline) &&
					md->status == SDF_EDF_MEM_RELEASED){
					next = md;
				}
		       ...
		       struct rq *rq)
{
	...
	DSTRM_EVENT(SDF_EDF, FUNC_ITERATOR_NEXT, 0);
	...
	DSTRM_EVENT_PRINTF(SDF_EDF, SCHED_CHOICE, next->deadline, "%s", 
			   next->member->ns->name);
...
static void sdf_edf_iterator_finalize(struct gsched_group *group,
				  ...
				  struct rq *rq)
{
	DSTRM_EVENT(SDF_EDF, FUNC_ITERATOR_FINALIZE, 0);
...
static void sdf_edf_create_group(struct gsched_group *group)
{
	...
	DSTRM_EVENT(SDF_EDF, FUNC_CREATE_GROUP, 0);       
	...
	gd->__mem_count = 0;
	gd->__ready_count = 0;
	gd->status = SDF_EDF_GROUP_NORESTRICT;
}
                             ...
                             struct gsched_member *member)
{
	...
	DSTRM_EVENT(SDF_EDF, FUNC_SETUP_MEMBER, 0);
				    ...
				    unsigned long deadline)
{
	...
	DSTRM_EVENT(SDF_EDF, FUNC_SORTED_INSERT_PRIV, 0);

        insert_before = list;

        list_for_each_entry(iter, list, sorted_ent) {
                if (iter->edf_member->deadline < deadline) {
...
static void __sdf_edf_insert_member(struct gsched_group *group,
		struct gsched_member *member, unsigned long deadline)
{
	struct edf_cpu *cd;
	struct edf_group *gd;
	struct edf_member *md;
	...
	DSTRM_EVENT(SDF_EDF, FUNC_INSERT_MEMBER_PRIV, 0);
...
{	
	...
	DSTRM_EVENT(SDF_EDF, FUNC_INSERT_MEMBER, 0);
...
static void __sdf_edf_remove_member(struct gsched_group *group,
	...
	DSTRM_EVENT(SDF_EDF, FUNC_REMOVE_MEMBER_PRIV, 0);
				  ...
				  struct gsched_member *member)
{
	...
	DSTRM_EVENT(SDF_EDF, FUNC_REMOVE_MEMBER, 0);
...
static void sdf_edf_release_member(struct gsched_group  *group,
				  struct gsched_member *member)
{
	DSTRM_EVENT(SDF_EDF, FUNC_RELEASE_MEMBER, current->pid); 
}
				...
				int newcpu)
{
	...
	DSTRM_EVENT(SDF_EDF, FUNC_MOVE_MEMBER, 0);
...
static void sdf_edf_release_all_members(struct edf_group *gd){
	...
	DSTRM_EVENT(SDF_EDF, FUNC_RELEASE_ALL_MEMBERS, 0);
				       ...
				       int next_status)
{
	...
	DSTRM_EVENT(SDF_EDF, FUNC_SET_MEMBER_STATUS, 0);
		...
		next_status == SDF_EDF_MEM_FINISHED )
	{
		...
		if (gd->status == SDF_EDF_GROUP_INIT && 
			  ...
			  void *param, size_t size)
{
	...
	DSTRM_EVENT(SDF_EDF, FUNC_SET_MEMBER_PARAMS, 0);
	...
	/* DSTRM_EVENT_PRINTF(SDF_EDF, SET_MEM_PARAMS, mp->cmd, "%s", member->ns->name); */

	switch(mp->cmd){
	case SDF_EDF_MP_CMD_DEADLINE:

		DSTRM_EVENT_PRINTF(SDF_EDF, SET_DEADLINE, mp->deadline, "%s", member->ns->name);
                                ...
                                void *param, size_t size)
{
	...
	DSTRM_EVENT(SDF_EDF, FUNC_SET_GROUP_PARAMS, 0);
	...
	case SDF_EDF_GP_CMD_NUM_MEMS:
		...
		gd->status = SDF_EDF_GROUP_INIT;
		DSTRM_EVENT(SDF_EDF, SET_GROUP_STATUS_INIT, 0);
		atomic_spin_unlock_irqrestore(&group->lock, flags);
