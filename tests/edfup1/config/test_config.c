	...
	*t_count = 0;
	...
	DL_FOREACH(thread_config, kc_elem){
		...
		HASH_ITER(hh, kc_elem->attributes, kc_attr, kc_tmp_attr) {
			...
			else if (!strcmp(kc_attr->name, "dl_sec"))
				tspec_elem->deadline.tv_sec = atol(kc_attr->value);				
			else if (!strcmp(kc_attr->name, "dl_nsec"))
				tspec_elem->deadline.tv_nsec = atol(kc_attr->value);				
			else if (!strcmp(kc_attr->name, "cpu"))
				 tspec_elem->cpu = atol(kc_attr->value);				 
			else
				printf("warning: unknown threadspec attribute %s", kc_attr->name);
		}
...
void pprint_threadspec(threadspec *head){
	...
	DL_FOREACH(head, ts) {
		...
		printf("intervals:\t%d\n", ts->intervals);
		printf("loop_count:\t%d\n", ts->loop_count);
		printf("deadline:\t%lds %ldns\n", (unsigned long)ts->deadline.tv_sec, ts->deadline.tv_nsec);  
		printf("cpu:\t\t%d\n", ts->cpu);
		printf("has next:\t%s\n", ts->next != NULL ? "yes" : "no");		
		printf("\n");       	       
...
static threadspec* get_new_threadspec()
{
	...
	ti->intervals = 0;
	ti->loop_count = 0;
	ti->deadline.tv_sec = 0;
	ti->deadline.tv_nsec = 0;
	ti->cpu = 0;
